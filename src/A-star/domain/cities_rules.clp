(defrule MAIN::start_city_TO (declare (salience 5))

?f1 <- (city_state
        (name ?name)  (qtyProd ?qtyProd) (objProd ?objProd)
                      (qtyNeed ?qtyNeed) (objNeed ?objNeed)
                      (qtyStore ?qtyStore) (objStore ?objStore)
      )

=>
    (bind ?id (sym-cat gen 1))
    (assert
        (find_heuristic_wares_costs) (find_heuristic_travel_costs)
        (status
          (ident ?id)
          (subject city)
          (data ?name  ?qtyProd  ?objProd  ?qtyNeed  ?objNeed  ?qtyStore  ?objStore))
        (current ?id))
)


(defrule MAIN::start_vehicle_TO (declare (salience 5))

 ?f1 <- (vehicle_state
         (type ?type) (capacity ?capacity) (location ?location) (id ?id)
       )
 ...
 =>
     ...
     (assert
         (find_heuristic_wares_costs) (find_heuristic_travel_costs)
        (status
          (ident (sym-cat gen 1)) (subject transport) (data plane 7 PA 6))
        (current ?id))
 )

 (defrule MAIN::halt (declare (salience 6))

  ?f1 <- (solved_goal (subject city) (data $?c1))
  =>
      (halt)
  )
